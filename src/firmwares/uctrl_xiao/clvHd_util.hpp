#ifndef _CLVHD_UTIL_HPP_
#define _CLVHD_UTIL_HPP_
...
class ClvHd
{
    ...
    begin()
    {
        ...
        for(int i = 0; i < NB_ADDRESS_PINS; i++) pinMode(m_addPins[i], OUTPUT);
        selectBrd(0xff);
    ...
    {
        for(unsigned i = 0; i < NB_ADDRESS_PINS; i++)
    ...
    const int m_addPins[NB_ADDRESS_PINS] = {0, 1, 2};
    int m_nbModule = 0;
    int m_availPin = D4;// SDA
    int m_clkPin = D5;// SCL
    int m_outPin = D7;// io1_serial
};

#endif // _CLVHD_UTIL_HPP_