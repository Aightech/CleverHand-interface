...
{
void
WifiController::clientThread(WifiClient *client)
{
    (void)client;
    // //check if the socket is still connected
    // int error = 0;
    // socklen_t len = sizeof(error);
    // Communication::SOCKET s = client->socket;
    // int retval = getsockopt(s, SOL_SOCKET, SO_ERROR, &error, &len);
    // logln("Client#" + std::to_string(s) + ": Thread started", true);
    // //while the socket is still connected
    // while(retval == 0 && error == 0)
    // {
    //     if(serverTCP.is_available(s))
    //     {
    //         logln("Client#" + std::to_string(s) + ": data available  [" +
    //                   std::to_string(serverTCP.is_available(s)) + "]",
    //               true);
    //     }
    //     //     uint8_t c = 0;
    //     //     serverTCP.read_byte(s, &c, 1, false, false);
    //     //     logln("Received command: " + std::to_string(c), true);
    //     //     switch(c)
    //     //     {
    //     //     case 'A':
    //     //     {
    ...
    //     //         uint8_t size = 0;
    //     //         serverTCP.read_byte(s, &c, 1, false, true);
